...
Thanks for wanting to contribute to MetaEdit. The best way to start is to use
...
as well as with automated tests. The repo ships an isolated Obsidian E2E harness
that provisions a throwaway vault and drives the real app via the `obsidian`
CLI; see `AGENTS.md` for the full workflow. The quickest check is:
...
Keep pull requests narrow. `main.js` is generated and attached to releases
rather than committed; commit `styles.css` when your change updates it.
