...
const INSTANCE = "/tmp/metaedit-obsidian-e2e/metaedit-wt-abc123def456";
...
const PS_FIXTURE = [
	...
	`  101   100 /Applications/Obsidian.app/Contents/Frameworks/Obsidian Helper (GPU).app/Contents/MacOS/Obsidian Helper (GPU) --type=gpu-process --user-data-dir=/private/tmp/metaedit-obsidian-e2e/metaedit-wt-abc123def456/home/Library/Application Support/obsidian`,
	`  102   100 /Applications/Obsidian.app/Contents/Frameworks/Obsidian Helper (Renderer).app/Contents/MacOS/Obsidian Helper (Renderer) --type=renderer --user-data-dir=/private/tmp/metaedit-obsidian-e2e/metaedit-wt-abc123def456/home/Library/Application Support/obsidian`,
	...
	"  400     1 /Applications/Obsidian.app/Contents/MacOS/Obsidian --user-data-dir=/tmp/metaedit-obsidian-e2e/metaedit-other-111111111111/home/Library/Application Support/obsidian --password-store=basic",
].join("\n");
...
describe("commandMatchesInstance", () => {
	it("matches both the /tmp and /private/tmp forms of the instance path", () => {
		...
		expect(
			commandMatchesInstance(
				"x --user-data-dir=/private/tmp/metaedit-obsidian-e2e/metaedit-wt-abc123def456/home/x",
				INSTANCE,
			),
		).toBe(true);
	});
	...
	it("does not match the dev vault, quickadd, or a sibling worktree", () => {
		...
		expect(
			commandMatchesInstance(
				"--user-data-dir=/tmp/metaedit-obsidian-e2e/metaedit-other-111111111111/home/x",
				INSTANCE,
			),
		).toBe(false);
	});
...
describe("orphan detection", () => {
	...
	it("reads the registered vault paths", async () => {
		const root = await makeTempDir("metaedit-profiles");
		const instance = await seedInstance(
			root,
			"metaedit-a-aaaaaaaaaaaa",
			"/x/y",
		);
	...
	it("is orphaned only when every backing vault is gone", async () => {
		...
		const live = await seedInstance(
			root,
			"metaedit-live-bbbbbbbbbbbb",
			liveVault,
		);
		const gone = await seedInstance(
			root,
			"metaedit-gone-cccccccccccc",
			"/does/not/exist",
		);
	...
	it("prefers the worktree marker: worktree-gone reaps even with a live vault; worktree-alive spares even with a missing vault", async () => {
		...
		const spared = await seedInstance(
			root,
			"metaedit-m1-aaaaaaaaaaaa",
			"/vault/leaf/gone",
		);
		await fs.writeFile(
			path.join(spared, "metaedit-e2e-instance.json"),
		...
		const leaked = await seedInstance(
			root,
			"metaedit-m2-bbbbbbbbbbbb",
			liveWorktree,
		);
		await fs.writeFile(
			path.join(leaked, "metaedit-e2e-instance.json"),
	...
	it("stays conservative when the registration is unreadable", async () => {
		const root = await makeTempDir("metaedit-profiles");
		const bare = path.join(root, "metaedit-bare-dddddddddddd");
...
describe("reapOrphanedInstances", () => {
	...
	it("continues reaping the rest of the scan when one orphan's teardown fails", async () => {
		...
		const bad = await seedInstance(
			root,
			"metaedit-bad-aaaaaaaaaaaa",
			"/gone/1",
		);
		const good = await seedInstance(
			root,
			"metaedit-good-bbbbbbbbbbbb",
			"/gone/2",
		);
		const removed: string[] = [];
		const logs: string[] = [];
