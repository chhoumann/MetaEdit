...
export class KanbanHelper extends OnFileModifyAutomator {
    private getBoards(): KanbanProperty[] {
    ...
    public async onFileModify(file: TFile): Promise<void> {
        const kanbanBoardFileContent: string = await this.app.vault.cachedRead(
            file,
        );
        const kanbanBoardFileCache: CachedMetadata | null =
        ...
        await this.updateFilesInBoard(
            links,
            targetBoard,
            kanbanBoardFileContent,
        );
    }

    private findBoardByName(boardName: string): KanbanProperty | undefined {
        return this.getBoards().find((board) => board.boardName === boardName);
    }

    private getLinkFile(link: LinkCache): TFile | undefined {
    ...
    private async updateFilesInBoard(
        ...
        for (const link of links) {
            const linkFile: TFile | undefined = this.getLinkFile(link);
            if (!linkFile) {
                log.logWarning(
                    `Link file not found for link ${link.link} in board ${board.boardName}`,
                );
                continue;
            }

            const linkIsMarkdownFile: boolean =
                !!abstractFileToMarkdownTFile(linkFile);
            if (!linkFile || !linkIsMarkdownFile) {
                log.logMessage(
                    `${link.link} is not updatable for the KanbanHelper.`,
                );
                return;
            }

            await this.updateFileInBoard(
                link,
                linkFile,
                board,
                kanbanBoardFileContent,
    ...
    private async updateFileInBoard(
        link: LinkCache,
        linkFile: TFile,
        board: KanbanProperty,
        kanbanBoardFileContent: string,
    ): Promise<void> {
        const heading: string | null = this.getTaskHeading(
            link.original,
            kanbanBoardFileContent,
        );
        if (!heading) {
            log.logWarning(
                'found linked file but could not get heading for task.',
            );
            return;
        }
        const parser = new MetaEditParser();
        const fileProperties: Property[] = await parser.getFileMetadata(
            linkFile,
        );
        if (!fileProperties) {
            log.logWarning(
                `No properties found in '${board.boardName}', cannot update '${board.property}'.`,
            );
            return;
        }
        const targetProperty = fileProperties.find(
            (prop) => prop.key === board.property,
        );
        if (!targetProperty) {
            log.logWarning(
                `'${board.property} not found in ${board.boardName} for file "${linkFile.name}".'`,
            );
            new Notice(
                `'${board.property} not found in ${board.boardName} for file "${linkFile.name}".'`,
            ); // This notice will help users debug "Property not found in board" errors.
            return;
        }
    ...
    private getTaskHeading(
        targetTaskContent: string,
        fileContent: string,
    ): string | null {
